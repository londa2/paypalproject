...
if($accion!=''){
    switch($accion){
        case 'agregar':
        $sql="INSERT INTO bebidas(id, producto, precio, ruta) VALUES(:id, :producto, :precio, :ruta)";
        ...
        break;
        case 'editar':
            $sql="UPDATE bebidas SET producto=:producto, precio=:precio, ruta=:ruta WHERE id=:id";
            $consulta=$conexionBD->prepare($sql);
            $consulta->bindParam(':id',$id);
            $consulta->bindParam('producto',$producto);
            $consulta->bindParam(':precio',$precio);
...
$consulta=$conexionBD->prepare("SELECT * FROM bebidas");
$consulta->execute();
